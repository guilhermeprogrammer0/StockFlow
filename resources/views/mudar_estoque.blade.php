        ...
        <x-area-component>
            <div class="formulario p-2 mt-3 bg-white rounded text-center">
                <div class="mt-2">
                    ...
                    @endif
                    @if(session('erro2'))
                    <div class="text-red-600 text-2xl bg-red-100 text-center">{{session('erro2')}}</div>
                    @endif
                </div>
                <div>
                    <h2 class="text-center color text-white text-3xl mt-3">Mudar estoque</h2>
                </div>
                <h5 class="text-center text-2xl"><strong>{{$produto->nome}} </strong></h5>
                ...
                <form action="{{route('mudar_estoque_submit')}}" method="post" novalidate>
                    ...
                    @if(session('erro'))
                    <div class="text-red-600">{{session('erro')}}</div>
                    @endif
                    <div class="mb-3">
